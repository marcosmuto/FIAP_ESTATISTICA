...
<li>Os testes foram parametrizados com um nível de confiança de 95%
...
dados_padronizados = as.data.frame(scale(VinhosBrancosNum))
...
```{r fig.width=16, fig.height=12,echo=FALSE, warning=FALSE, message=FALSE}
...
plot(pc,labels=names(dados_padronizados))
...
treinamento %>%
  select(fixedacidity, volatileacidity, citricacid, chlorides, pH, sulphates, contribso2, acucaralcool, quality) -> treinamento_comPCA

teste %>% 
  select(fixedacidity, volatileacidity, citricacid, chlorides, pH, sulphates, contribso2, acucaralcool, quality) -> teste_comPCA
